...
class Pedometer:
  ...
  def process(self):
    ...
    # ...
    # ...
    x_filtered = filt.filter(self.__b, self.__a, x)  # Using filtfilt for zero-phase filtering

    # Apply moving average (if used in Tutorial 2)
    x_smooth = filt.moving_average(x_filtered, 20)  # Window size is an example, adjust as necessary

    # Detrend the signal (if detrending was beneficial in Tutorial 2)
    x_detrended = filt.detrend(x_smooth, 50)  # Window size is an example, adjust as necessary
